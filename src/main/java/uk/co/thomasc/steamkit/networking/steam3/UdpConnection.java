...
public class UdpConnection extends Connection {
    ...
    public void connect(InetSocketAddress endPoint, int timeout) {
        ...
        DebugLog.writeLine("UDP Connection", "connecting to " + endPoint);
    ...
    private void sendPacket(UdpPacket packet) {
        ...
        DebugLog.writeLine("UDP Connection", String.format("Sent -> %s Seq %d Ack %d; %d bytes; Message: %d bytes %d packets",
        ...
        try {
            ...
            DebugLog.writeLine("UDP Connection", "Critical socket failure: %s", e);
    ...
    private void sendPendingMessages() {
        synchronized (outPackets) {
            if (System.currentTimeMillis() > nextResend && outSeqSent > outSeqAcked) {
                ...
                DebugLog.writeLine("UDP Connection", "Sequenced packet resend required");
    ...
    private boolean dispatchMessage() {
        ...
        DebugLog.writeLine("UDP Connection", "Dispatchin message: %s bytes", data.length);
    ...
    private void receivePacket(UdpPacket packet) {
        ...
        DebugLog.writeLine("UDP Connection", "<- Recv'd %s Seq %d Ack %d; %d bytes; Message: %d bytes %d packets",
                packet.getHeader().getPacketType(), packet.getHeader().getSeqThis(), packet.getHeader().getSeqAck(),
                packet.getHeader().getPayloadSize(), packet.getHeader().getMsgSize(), packet.getHeader().getPacketsInMsg());
        ...
        switch (packet.getHeader().getPacketType()) {
            ...
            case Data:
                ...
                break;
            case Disconnect:
                DebugLog.writeLine("UDP Connection", "Disconnected by server");
                state.set(State.DISCONNECTED);
                return;
            case Datagram:
                break;
            default:
                DebugLog.writeLine("UDP Connection", "Received unexpected packet type " + packet.getHeader().getPacketType());
                break;
        }
    }
    ...
    private void receiveAccept(UdpPacket packet) {
        ...
        DebugLog.writeLine("UDP Connection", "Connection established");
        ...
        public void run() {
            ...
            while (state.get() != State.DISCONNECTED) {
                try {
                    try {
                        ...
                        if (System.currentTimeMillis() > timeout) {
                            DebugLog.writeLine("UDP Connection", "Connection timed out");
                            state.set(State.DISCONNECTED);
                            break;
                        }
                    }
                    ...
                    DebugLog.writeLine("UDP Connection", "Exception while reading packer");
                    state.set(State.DISCONNECTED);
                    break;
                }
                ...
                if (state.get() == State.DISCONNECTING && outPackets.isEmpty()) {
                    DebugLog.writeLine("UDP Connection", "Graceful disconnect completed");
                    state.set(State.DISCONNECTED);
                    userRequestDisconnect = true;
                    break;
                }
            ...
            DebugLog.writeLine("UDP Connection", "Celling onDisconnected");
